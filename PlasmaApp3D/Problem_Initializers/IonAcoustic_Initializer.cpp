...
IonAcoustic_Initializer::IonAcoustic_Initializer(PlasmaData* pdata_in)
{
		...
		v_shift = sqrt(1.0/pdata->mspecies[1]*1.0*pdata->Te/(1.0+1.0/(kt*kt)) + 1.0*2.0e-4/pdata->mspecies[1]);
									 ...
									 realkind& vx, realkind& vy, realkind& vz,int ispecies, int iptcl)
{
	...
	idist = iptcl_new%(pdata->nptcls_cpu*pdata->num_nodes/20);
	int ivel = iptcl_new/(pdata->nptcls_cpu*pdata->num_nodes/20);
	double temp_x;


//	px = distribution_intrp((idist+0.5)/(0.5*pdata->nptcls_cpu*pdata->num_nodes),0.0,pdata->Lx,cdf);

	px = distribution_intrp((idist)/(1/20.0*pdata->nptcls_cpu*pdata->num_nodes),0.0,pdata->Lx,cdf)+0.25*pdata->Lx;
...
//	realkind ux = alpha*v_shift*sin((px+ix)*pdata->dxdi/kt) + v_shift;
	...
	if(ispecies == 0)
		vx = box_muller(ux,sqrt(2.0*pdata->Te/(pdata->mspecies[ispecies]*mass_e)));
	else
...
void IonAcoustic_Initializer::initialize_particles(ParticleList* particles,
										  HOMoments* moments,
										  ParallelInfo* myinfo)
{
...
void IonAcoustic_Initializer::initialize_fields(FieldData** fields,
										HOMoments** moments,
										ParallelInfo* myinfo)
{
...
void IonAcoustic_Initializer::check_step(ParticleList* particles,
