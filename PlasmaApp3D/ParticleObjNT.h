...
public:
	...
	__device__
	ParticleObjNT& operator=(const ParticleList& list_in)
	{
	...
	__device__
	void write_back_all(ParticleList& list_in)
	{
...
//		list_in.num_piccard2[pid[iptcl]] += npiccard2(iptcl);
	...
	__device__
	void copy_in_all(const ParticleList& list_in)
	{
...
//		npiccard2(iptcl) = 0;
	...
	__device__
	void copy_in_gpu(const ParticleListGPU& list_in, const int iptcl)
	{
...
//		npiccard2(iptcl) = 0;
	...
	__device__ __attribute__((noinline))
	void PicardIterate(PlasmaData* pdata, FieldData* fields, CurrentTally* currents,typevecN<realkind,N>& dtau0);
	__device__
	...
	__device__
	typevecN<typevecN<realkind,N>,nVel> get_accel(FieldData* fields,
								const typevecN<typevecN<realkind,N>,nSpatial>& x,
								const typevecN<typevecN<realkind,N>,nVel>& v);

	__device__
	typevecN<typevecN<realkind,N>,nVel> get_accel2(FieldData* fields,
											const typevecN<typevecN<realkind,N>,nSpatial>& x,
											typevecN<realkind,N> dtau);

	template<enum FieldData_deriv ideriv> __device__
	typevecN<typevecN<realkind,N>,nVel> get_B(FieldData* fields,
			const typevecN<typevecN<realkind,N>,nSpatial>& x,
			const typevecN<typevecN<int,N>,nSpatial>& ix);

	template<enum FieldData_deriv ideriv> __device__
	typevecN<typevecN<realkind,N>,nVel> get_E(FieldData* fields,
			const typevecN<typevecN<realkind,N>,nSpatial>& x,
			const typevecN<typevecN<int,N>,nSpatial>& ix);
	__device__
...
template<const int ileft,const int nSpatial,const int nVel,const bool iEM> __attribute__((noinline)) __host__
