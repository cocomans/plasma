...
void ParticleListCPU::copy_from(const ParticleList* list_in)
{
...
void ParticleListCPU::allocate(int nptcls_in)
{
...
void ParticleListCPU::init(ProblemInitializer* initializer, HOMoments* moments)
{
	...
	StressTally stress(&moments->get_val(0,0,0,ispecies,HOMoments_S2),
						  make_int3(moments->pdata->nx,moments->pdata->ny,moments->pdata->nz),
						  moments->pdata->dxdi,moments->pdata->dydi,moments->pdata->dzdi,
						  moments->pdata->ndimensions);
	...
	for(int i=0;i<nptcls;i++)
	{
		...
		currents.tally(px,py,pz,vx,vy,vz,ix,iy,iz,1.0);

		charge.tally(px,py,pz,
				ix,iy,iz,
				1.0);

		stress.tally(px,py,pz,
				vx,vy,vz,
				ix,iy,iz,
				1.0);
...
long long int ParticleListCPU::push(PlasmaData* pdata, FieldData* fields, HOMoments* moments)
{
	return push_interface<VEC_LENGTH_MAX>(pdata,fields,moments);
}

template<int VEC_LENGTH>
long long int ParticleListCPU::push_interface(PlasmaData* pdata, FieldData* fields, HOMoments* moments)
{
	if(pdata->cpu_vec_length == VEC_LENGTH)
	{
		return pushT<VEC_LENGTH>(pdata,fields,moments);
	}
	else
	{
		return push_interface<VEC_LENGTH-1>(pdata,fields,moments);
	}
...
template<>
long long int ParticleListCPU::push_interface<1>(PlasmaData* pdata, FieldData* fields, HOMoments* moments)
{
	if(pdata->cpu_vec_length == 1)
	{
		return pushT<1>(pdata,fields,moments);
	}

}

template<int VEC_LENGTH> __attribute__ ((noinline))
long long int ParticleListCPU::pushT(PlasmaData* pdata, FieldData* fields, HOMoments* moments)
{
...
//	        std::cout<<os.str()<<std::flush;
		...
		StressTally stress(&my_moment->get_val(0,0,0,ispecies,HOMoments_S2),
							  make_int3(moments->pdata->nx,moments->pdata->ny,moments->pdata->nz),
							  moments->pdata->dxdi,moments->pdata->dydi,moments->pdata->dzdi,
							  moments->pdata->ndimensions);
