...
JFNK2D::JFNK2D( const Teuchos::RCP<LowOrderProblem2D>& Problem,
...
void JFNK2D::UpdateSolution(const Teuchos::RCP<EpVecWrapper>& v)
{
...
void JFNK2D::UpdateSolution(const Teuchos::RCP<Epetra_Vector>& v, double Target_Resid)
{
...
void JFNK2D::GetSolution(const Teuchos::RCP<EpVecWrapper>& X_out)
{
...
double JFNK2D::ComputeResidualNorm()
{
...
void JFNK2D::ComputeResidual(const Teuchos::RCP<EpVecWrapper>& v)
{
...
void JFNK2D::DumpSolution(int timestep)
{
  // Extract solution
  const NOX::Epetra::Group &finalGroup =
    dynamic_cast<const NOX::Epetra::Group&> (nonLinearSolver->getSolutionGroup());
...
//  // Build a combo of stopping criteria in OR mode
  ...
  Teuchos::RCP<NOX::StatusTest::NormF> relRes =
    Teuchos::rcp(new NOX::StatusTest::NormF(*group, 1.0e-8) );
  ...
  combo->addStatusTest(relResUpdate);
