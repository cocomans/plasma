...
//		Ex1 = 0.0;
		...
		particles_temp -> allocate(pdata,pdata->nptcls);
		particles_temp -> ispecies = 0;
		...
		vx0 = (realkind*)malloc(pdata->nptcls*sizeof(realkind));
		vy0 = (realkind*)malloc(pdata->nptcls*sizeof(realkind));

		x0 = (realkind*)malloc(pdata->nptcls*sizeof(realkind));
		y0 = (realkind*)malloc(pdata->nptcls*sizeof(realkind));

		xt = (float*)malloc(pdata->nptcls*nsteps*sizeof(float));
		yt = (float*)malloc(pdata->nptcls*nsteps*sizeof(float));
		xrt = (float*)malloc(pdata->nptcls*nsteps*sizeof(float));
		yrt = (float*)malloc(pdata->nptcls*nsteps*sizeof(float));
...
void ExB_Initializer::initialize_particles(NodeParticleList* particles,
										  NodeHOMoments* moments)
{
	...
	particles_temp -> copy_from(particles->cpu_particles);
...
void ExB_Initializer::initialize_fields(FieldDataCPU* fields,
										NodeHOMoments* moments)
{
...
void ExB_Initializer::check_step(NodeParticleList* particles_next,
		NodeHOMoments* moments,FieldDataCPU* fields_old,FieldDataCPU* fields_next)
{
	...
	particles_temp -> copy_from(particles_next->cpu_particles);
...
void ExB_Initializer::finish(NodeParticleList* particles,
		NodeHOMoments* moments,NodeFieldData* fields_half)
{
	...
	for(int i=0;i<(int)(fmin(2,pdata->nptcls));i++)
	{
		if(pdata->ndimensions == 1)
