...
#ifdef shitty_timer
#define calib_iter 10
#else
#define calib_iter NANO_SECONDS_IN_SEC/50
#endif
...
public:
	...
	//@{
	uint64_t ts0, ts1, ts_cumm,ts_last;
	//@}
	...
	*/
	...
	void CalibrateTicks()
	{
	  ...
	  uint64_t i;
	  double temp = 0;
	  for (i = 0; i < calib_iter; i++) temp+=sqrt((double)i); /* must be CPU intensive */
...
//		ts_cumm += ts1-ts0;
	...
	{
		ts_cumm = 0;
		cummulative.tv_nsec  = 0;
		cummulative.tv_sec = 0;
	}
...
//		return cummulative.tv_nsec*1.0e-6 + cummulative.tv_sec*1.0e3;
	...
	*/
	...
	double getLastT()
	{
//		return difference.tv_nsec*1.0e-6 + difference.tv_sec*1.0e3;

		return 1.0e-6*((double)ts_last)/g_TicksPerNanoSec;
