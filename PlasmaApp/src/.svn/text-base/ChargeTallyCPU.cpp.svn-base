...
ChargeTallyCPU::ChargeTallyCPU(realkind* charge_in,
						   int3 dims_in,
...
ChargeTallyCPU::ChargeTallyCPU(){};



void ChargeTallyCPU::tally1d(const realkind px,
		 const int ix_in,
...
void ChargeTallyCPU::tally2d(const realkind px,const realkind py,
	...
	// x component
		...
		for(int j=-2;j<3;j++)
		{
			for(int i=-2;i<3;i++)
			{
				...
				temp = S2_shape(xp)*S2_shape(yp)*vol_inv;
...
void ChargeTallyCPU::tally3d(const realkind px,const realkind py,const realkind pz,
...
void ChargeTallyCPU::tally(const realkind px, const realkind py, const realkind pz,
						 const int ix_in, const int iy_in, const int iz_in,
						 const realkind scale)
{
