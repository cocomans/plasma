...
public:
	...
	virtual void initialize_particles(NodeParticleList* particles, NodeHOMoments* moments){};

	virtual void initialize_fields(FieldDataCPU* fields, NodeHOMoments* moments){};
	...
	virtual void check_step(NodeParticleList* particles_next,
			NodeHOMoments* moments,FieldDataCPU* fields_old,FieldDataCPU* fields_next){}

	virtual void finish(NodeParticleList* particles,
...
public:
	ewave_2d(const double _kt,
					   ...
					   const double _y0,
					   const int 	_nx,
					   const int 	_ny) : kt(_kt),
	alpha(_alpha),nx(_nx),ny(_ny),Lx(_Lx),Ly(_Ly),x0(_x0),y0(_y0)
	{
		pdf = (double*)malloc((nx*ny+1)*sizeof(double));
		pdf[0] = 0;
